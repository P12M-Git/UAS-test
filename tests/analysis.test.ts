...
test("green flag filtering recognises SF",()=>{const d=parseTimingCsv(csv([row(1,1,90,90,"A","SF"),row(1,2,120,210,"A","SF")]),"race.csv");assert.equal(d.laps[0].green,true);assert.equal(d.laps[1].green,false)});
...
test("category aggregation averages driver-level metrics",()=>{const base:any={category:"Gold",clean:20,avgBest:90,avgAll:91,best:89,stdBest:1,stdAll:2,madBest:.5,madAll:1,zAll:.2};const r=categoryMetrics([{...base},{...base,avgBest:94,avgAll:95,best:92,zAll:.4}]);assert.equal(r[0].avgBest,92);assert.ok(Math.abs(r[0].avgZ-.3)<1e-12);assert.equal(mean([90,94]),92)});
