...
export default function Results({laps}:{laps:Lap[]}){
  ...
  return <>
    ...
    <div className="section-tabs">{rounds.map(r=><label key={r.key}><input type="checkbox" checked={!hidden.includes(r.key)} onChange={e=>setHidden(old=>e.target.checked?old.filter(k=>k!==r.key):[...old,r.key])}/>{r.event} · {r.session}</label>)}</div>
    ...
    {!selected.length?<p>Select at least one event.</p>:<div className="results-scroll"><table className="results-table"><thead><tr><th>DRIVER</th>{selected.map(r=><th key={r.key}>{r.event}<br/><small>{r.session}</small></th>)}</tr></thead>
